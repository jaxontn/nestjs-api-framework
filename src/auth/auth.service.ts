...
export class AuthService {
  ...
  async customerLookup(phoneOrEmail: string, merchantId: string): Promise<any> {
    try {
      ...
      const sessionToken = await this.createCustomerSession(customer.id);
  ...
  // Get customer by phone/email (for direct access)
    ...
    try {
      ...
      const existingCustomer = await this.customerRepository.findOne({
        where: [
          ...
          { merchant_id: merchantId, email, is_active: true },
        ],
      });
  ...
  private async calculateCustomerMetrics(customer: Customer): Promise<any> {
    try {
      ...
      const completedSessions = await this.gameSessionRepository.count({
        where: { customer_id: customer.id, was_completed: true },
      });
      ...
      const totalPrizesWon = await this.leaderboardRepository.count({
        where: { customer_id: customer.id, achievement: Like('%') }, // Not null
      });
